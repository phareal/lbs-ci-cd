...
test('PUT /api/users/:id met à jour le nom', async () => {
  const createResponse = await request(app)
    .post('/api/users')
    .send({ name: 'Ancien Nom', email: 'update@example.com' });
...
test('PUT /api/users/:id retourne 400 si aucun champ fourni', async () => {
  const createResponse = await request(app)
    .post('/api/users')
    .send({ name: 'Nom', email: 'no-fields@example.com' });
  ...
  expect(response.status).toBe(400);
  expect(response.body.message).toBe('Au moins un champ (name ou email) doit être fourni');
});
...
test('DELETE /api/users/:id supprime un utilisateur', async () => {
  const createResponse = await request(app)
    .post('/api/users')
    .send({ name: 'Jean', email: 'delete@example.com' });
...
test('Route inconnue retourne 404', async () => {
  ...
  expect(response.status).toBe(404);
  expect(response.body.message).toBe('Ressource non trouvée');
});
